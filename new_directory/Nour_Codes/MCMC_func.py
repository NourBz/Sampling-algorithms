...
def lpost(data_dl, delta_dl, H0_prop, Omega_m_prop, param1_low_range = 30, param1_high_range = 100, param2_low_range = 0, param2_high_range = 5, **kwargs):
    ...
    param2 (float): Value of parameter 2.
    param2_low_range (float, optional): Lower bound of the range for parameter 2. Default is 0.
    param2_high_range (float, optional): Upper bound of the range for parameter 2. Default is 5.
...
def MCMC_run(data_dl, delta_dl, param_start,  printerval = 5000, H0_var_prop = 0.05, 
    ...
    Omega_m_chain = [param_start[1]]

    # Initial value for log posterior
    lp = []
    lp.append(lpost(data_dl, delta_dl, H0_chain[0], Omega_m_chain[0], **kwargs)) # Append first value of log posterior 
    ...
    for i in tqdm(range(1, Ntotal)):
        ...
        Omega_m_prop = np.random.normal(Omega_m_chain[i - 1], np.sqrt(Omega_m_var_prop))
    
        # Compute log posterior
        lp_prop = lpost(data_dl, delta_dl, H0_prop, Omega_m_prop, **kwargs)
        
        if accept_reject(lp_prop, lp_prev) == 1:  # Accept
            H0_chain.append(H0_prop)    # accept H0_{prop} as new sample
            Omega_m_chain.append(Omega_m_prop) 
            ...
            Omega_m_chain.append(Omega_m_chain[i - 1])
    ...
    Omega_m_chain = np.array(Omega_m_chain)
    H0_chain      = H0_chain[burnin:]
    Omega_m_chain = Omega_m_chain[burnin:]
    
    return H0_chain, Omega_m_chain, lp  # Return chains and log posterior.    
